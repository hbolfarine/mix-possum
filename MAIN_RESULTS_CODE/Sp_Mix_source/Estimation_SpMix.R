...
MultVar_NormMixt_Gibbs_IndPriorNormalgamma <- function(y, S_0, mu_0, sigma_0, eta_0, e0, c0, C0_0, 
                                                       g0, G0, b0, B0, nu, B_0, M, burnin, c_proposal, priorOnE0, lambda, R) {
  ...
  while (m <= M | m <= burnin) {
    ...
    for (k in 1:K) {
      ...
      sig <- bayesm::rwishart(2 * ck[k], 0.5 * chol2inv(chol(Ck[, , k])))  #attention: rwishart(nu,v)(Rossi)=> nu=2*c0,v=0.5*C0, wishart(c0,C0) (FS)
      sigma_j[, , k] <- sig$IW
      invsigma_j[, , k] <- sig$W
